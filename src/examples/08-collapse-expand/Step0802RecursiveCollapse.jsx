/**
 * ================================================================
 * [초보자용 상세 주석] 08-02 모든 하위 노드 재귀 접기
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 손자 이하 모든 하위 노드와 연결선을 재귀적으로 숨깁니다.
 * - 예상 결과: 노드 5개, 연결선 4개
 ...
 * - destination: 이번 예제에서 destination 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
 *
 * 기억할 점
...
new Rect(x, y, 48, 24); n.text = text; n.expandable = expandable; n.expanded = expandable; model.addItem(n); return n; }; const root = make("부모", 94, 10, true), a = make("자식 A", 42, 78, true), b = make("자식 B", 146, 78); root.tag = "root"; model.factory./* [연결선 생성] 출발 노드와 도착 노드가 Diagram에 준비된 뒤 두 노드를 연결합니다. */
createDiagramLink(root, a); model.factory.createDiagramLink(root, b); [make("손자 1", 15, 148), make("손자 2", 70, 148)].forEach((n) => model.factory.createDiagramLink(a, n)); return model; });
  ...
  return <div data-testid="diagram-demo"><button onClick={collapse}>손자까지 재귀 접기</button><span data-testid="tree-state"> {collapsed ? "모든 하위 노드 숨김" : "전체 펼침"}</span><div style={{ height: 460 }}>/* [화면 렌더링] 준비한 Diagram 모델과 ref, 이벤트 함수를 DiagramView prop으로 전달합니다. 부모 요소에는 반드시 높이가 있어야 합니다. */
