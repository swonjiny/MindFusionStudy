...
선택 노드 상세 패널 단계의 목적과 ControlNode 기반 구현 흐름을 이해한다. 데이터 수와 실제 Diagram 모델·DOM 수가 일치하는지도 확인한다.
...
우측 상세 패널을 선택 상태와 연동한다. 공통 기능은 `CityTreeCanvas`에 유지하고 이 메뉴는 `variant="panel"`만 선택한다.
...
```jsx
/** [초보자용 상세 주석] 선택 노드 상세 패널 단계의 독립 실행 진입점입니다. */
import CityTreeCanvas from "../../components/city-tree/CityTreeCanvas";
export default function Step1924CityTreeDetailPanel(props) {
  return <CityTreeCanvas {...props} variant="panel" />;
}
```
...
다음 메뉴에서 전체 및 루트별 열기·닫기 기능을 추가한다.
