/**
 * ================================================================
 * [초보자용 상세 주석] 07-01 세로 트리 자동 배치
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - TreeLayout으로 위에서 아래 방향의 트리를 자동 배치합니다.
 * - 예상 결과: 노드 4개, 연결선 3개
 ...
 * - TopToBottom: 이번 예제에서 TopToBottom 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
 *
 * 기억할 점
...
new Rect(0, 0, 48, 26); n.text = text; n.expandable = expandable; n.expanded = expandable; model.addItem(n); return n; };
    const root = make("루트", true); ["자식 A", "자식 B", "자식 C"].forEach((text) => model.factory./* [연결선 생성] 출발 노드와 도착 노드가 Diagram에 준비된 뒤 두 노드를 연결합니다. */
createDiagramLink(root, make(text))); const layout = new TreeLayout(); layout.direction = LayoutDirection.TopToBottom; layout.levelDistance = 28; layout.nodeDistance = 18; model.arrange(layout); return model;
  });
