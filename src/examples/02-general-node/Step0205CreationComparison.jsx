/**
 * ================================================================
 * [초보자용 상세 주석] 02-05 두 생성 방식 비교
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - Factory 방식과 직접 생성 방식을 한 화면에서 비교합니다.
 * - 예상 결과: 노드 2개, 연결선 0개
...
new Diagram();
    const factoryNode = model.factory./* [Factory 생성] Factory 메서드는 노드를 만들면서 Diagram에 자동 등록합니다. 반환 노드를 다시 addItem하지 않습니다. */
createShapeNode(24, 42, 55, 28);
    factoryNode.text = "Factory 생성"; factoryNode.brush = "#e8f8ef"; factoryNode.stroke = "#289766";
    const directNode = /* [일반 노드 생성] ShapeNode 객체만 만든 상태이며, bounds·text·스타일을 설정한 뒤 Diagram에 등록해야 화면에 나타납니다. */
new ShapeNode(model);
    directNode.bounds = /* [위치와 크기] Rect(x, y, width, height)로 다이어그램 좌표상의 위치와 노드 크기를 함께 지정합니다. */
new Rect(92, 42, 55, 28);
