/**
 * ================================================================
 * [초보자용 상세 주석] 08-04 전체 트리 접기
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 모든 가지를 접어 최상위 루트만 표시합니다.
 * - 예상 결과: 노드 7개, 연결선 6개
 ...
 *
 * 이번 예제의 핵심 용어
 * - Diagram.nodes: 이번 예제에서 Diagram.nodes 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
...
new Rect(x, y, 44, 23); n.text = text; n.expandable = expandable; n.expanded = expandable; model.addItem(n); return n; }; const root = make("루트", 100, 8, true), a = make("A", 48, 75, true), b = make("B", 152, 75, true); root.tag = "root"; model.factory./* [연결선 생성] 출발 노드와 도착 노드가 Diagram에 준비된 뒤 두 노드를 연결합니다. */
createDiagramLink(root, a); model.factory.createDiagramLink(root, b); [make("A1", 18, 145), make("A2", 70, 145)].forEach((n) => model.factory.createDiagramLink(a, n)); [make("B1", 130, 145), make("B2", 182, 145)].forEach((n) => model.factory.createDiagramLink(b, n)); return model; });
  ...
  return <div data-testid="diagram-demo"><button onClick={collapseAll}>전체 접기</button><span data-testid="tree-state"> {done ? "루트만 표시" : "전체 펼침"}</span><div style={{ height: 460 }}>/* [화면 렌더링] 준비한 Diagram 모델과 ref, 이벤트 함수를 DiagramView prop으로 전달합니다. 부모 요소에는 반드시 높이가 있어야 합니다. */
