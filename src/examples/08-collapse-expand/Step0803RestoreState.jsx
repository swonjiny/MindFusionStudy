/**
 * ================================================================
 * [초보자용 상세 주석] 08-03 기존 펼침 상태 복원
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 접기 전 visible·expanded 상태를 저장했다가 복원합니다.
 * - 예상 결과: 노드 4개, 연결선 3개
...
new Rect(x, y, 48, 24); n.text = text; n.expandable = expandable; n.expanded = expandable; model.addItem(n); return n; }; const root = make("부모", 94, 10, true), child = make("자식", 55, 80, true), sibling = make("형제", 145, 80), grandchild = make("손자", 55, 150); root.tag = "root"; model.factory./* [연결선 생성] 출발 노드와 도착 노드가 Diagram에 준비된 뒤 두 노드를 연결합니다. */
createDiagramLink(root, child); model.factory.createDiagramLink(root, sibling); model.factory.createDiagramLink(child, grandchild); return model; });
  ...
  return <div data-testid="diagram-demo"><button onClick={collapse}>임시 접기</button> <button onClick={restore}>기존 상태 복원</button><span data-testid="tree-state"> {label}</span><div style={{ height: 450 }}>/* [화면 렌더링] 준비한 Diagram 모델과 ref, 이벤트 함수를 DiagramView prop으로 전달합니다. 부모 요소에는 반드시 높이가 있어야 합니다. */
