/**
 * ================================================================
 * [초보자용 상세 주석] 13-03 선택 해제 시 크기 복원
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 선택을 해제하면 카드 목록을 숨기고 원래 크기로 복원합니다.
 * - 예상 결과: 노드 1개, 연결선 0개
 ...
 *
 * 이번 예제의 핵심 용어
 * - Selection.clear: 이번 예제에서 Selection.clear 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
 * - Rect: x, y 위치와 width, height 크기를 함께 나타냅니다.
 * - innerHTML: 이번 예제에서 innerHTML 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
 *
 * 기억할 점
...
new Diagram()); const viewRef = useRef(null); const initialized = useRef(false); const nodeRef = useRef(null); const [expanded, setExpanded] = useState(true); const events = useRef(0);
...
new ControlNode(coreView); node.template = expandedHtml(); node.bounds = /* [위치와 크기] Rect(x, y, width, height)로 다이어그램 좌표상의 위치와 노드 크기를 함께 지정합니다. */
new Rect(54, 8, 126, 78); nodeRef.current = node; /* [모델에 등록] 직접 new로 만든 노드는 addItem을 호출해야 Diagram이 관리하고 DiagramView가 그릴 수 있습니다. */
diagram.addItem(node); queueMicrotask(() => diagram.selection.addItem(node)); };
...
const selectionChanged = () => { const node = nodeRef.current; const content = node?.getContent?.(); if (!content) return; const selected = diagram.selection.nodes.includes(node); events.current += 1; content.innerHTML = selected ? expandedHtml() : collapsedHtml; node.bounds = selected ? new Rect(54, 8, 126, 78) : new Rect(65, 40, 110, 34); diagram.invalidate(); setExpanded(selected); report(selected); };
  ...
  return <div data-testid="diagram-demo"><p data-testid="expand-state">{expanded ? "선택 1개 · 확장 상태" : "선택 0개 · 원래 크기 복원"}</p><button type="button" onClick={() => diagram.selection.clear()}>선택 해제</button><div style={{ height: 430 }}>/* [화면 렌더링] 준비한 Diagram 모델과 ref, 이벤트 함수를 DiagramView prop으로 전달합니다. 부모 요소에는 반드시 높이가 있어야 합니다. */
