/**
 * ================================================================
 * [초보자용 상세 주석] 07-05 자식 추가 후 재배치
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 새 자식을 추가한 뒤 전체 트리를 다시 배치합니다.
 * - 예상 결과: 노드 6개, 연결선 5개
 ...
 *
 * 이번 예제의 핵심 용어
 * - addItem: 이번 예제에서 addItem 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
 * - Diagram.arrange: 이번 예제에서 Diagram.arrange 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
 *
 * 기억할 점
...
new Rect(0, 0, 46, 24); n.text = text; n.expandable = expandable; n.expanded = expandable; model.addItem(n); return n; }; const root = make("루트", true); root.tag = "root"; for (let i = 1; i <= 4; i += 1) model.factory./* [연결선 생성] 출발 노드와 도착 노드가 Diagram에 준비된 뒤 두 노드를 연결합니다. */
createDiagramLink(root, make(`자식 ${i}`)); return model;
  });
  ...
  const addAndArrange = () => { if (added) return; const n = new ShapeNode(diagram); n.bounds = new Rect(0, 0, 46, 24); n.text = "새 자식"; /* [모델에 등록] 직접 new로 만든 노드는 addItem을 호출해야 Diagram이 관리하고 DiagramView가 그릴 수 있습니다. */
diagram.addItem(n); diagram.factory.createDiagramLink(diagram.nodes.find((item) => item.tag === "root"), n); arrange(); setAdded(true); report(6); };
  return <div data-testid="diagram-demo"><button onClick={addAndArrange} disabled={added}>자식 추가 후 재배치</button><span data-testid="layout-state"> {added ? "6개 재배치 완료" : "5개 배치 완료"}</span><div style={{ height: 460 }}>/* [화면 렌더링] 준비한 Diagram 모델과 ref, 이벤트 함수를 DiagramView prop으로 전달합니다. 부모 요소에는 반드시 높이가 있어야 합니다. */
