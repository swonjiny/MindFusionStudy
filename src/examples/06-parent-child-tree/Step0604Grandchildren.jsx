/**
 * ================================================================
 * [초보자용 상세 주석] 06-04 손자 노드가 있는 트리
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 자식이 다시 부모가 되는 손자 구조를 만듭니다.
 * - 예상 결과: 노드 5개, 연결선 4개
...
new Rect(x, y, 50, 26); n.text = text; n.expandable = expandable; n.expanded = expandable; model.addItem(n); return n; };
    const root = make("부모", 92, 12, true), childA = make("자식 A", 40, 80, true), childB = make("자식 B", 144, 80);
    [childA, childB].forEach((child) => model.factory./* [연결선 생성] 출발 노드와 도착 노드가 Diagram에 준비된 뒤 두 노드를 연결합니다. */
createDiagramLink(root, child)); [make("손자 A-1", 16, 150), make("손자 A-2", 76, 150)].forEach((grandchild) => model.factory.createDiagramLink(childA, grandchild)); return model;
  });
