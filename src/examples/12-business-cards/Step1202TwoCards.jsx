/**
 * ================================================================
 * [초보자용 상세 주석] 12-02 명함형 카드 2개
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 배열 데이터 두 건을 하나의 HTML 노드에 표시합니다.
 * - 예상 결과: 노드 1개, 연결선 0개
 ...
 * - data-card-id: 이번 예제에서 data-card-id 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
 *
 * 기억할 점
...
new ControlNode(coreView); node.template = `<section data-mf-card-node="12-02" style="box-sizing:border-box;width:100%;height:100%;display:grid;gap:8px;padding:10px;border:2px solid #2877de;border-radius:14px;background:#f7fbff">${cards.map(cardHtml).join("")}</section>`; node.bounds = /* [위치와 크기] Rect(x, y, width, height)로 다이어그램 좌표상의 위치와 노드 크기를 함께 지정합니다. */
new Rect(58, 20, 115, 54); /* [모델에 등록] 직접 new로 만든 노드는 addItem을 호출해야 Diagram이 관리하고 DiagramView가 그릴 수 있습니다. */
...
const domCreated = (_sender, args) => report(args.node.getContent().querySelectorAll("[data-card-id]").length);
  ...
  return <div data-testid="diagram-demo"><p data-testid="card-summary">배열 데이터 {cards.length}개 · 카드 2개</p><div style={{ height: 460 }}>/* [화면 렌더링] 준비한 Diagram 모델과 ref, 이벤트 함수를 DiagramView prop으로 전달합니다. 부모 요소에는 반드시 높이가 있어야 합니다. */
