/**
 * ================================================================
 * [초보자용 상세 주석] 05-04 화살표 머리 설정
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - ArrowHeads.Triangle로 연결 방향을 표시합니다.
 * - 예상 결과: 노드 2개, 연결선 1개
 ...
 *
 * 이번 예제의 핵심 용어
 * - ArrowHeads: 이번 예제에서 ArrowHeads 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
 * - headShape: 이번 예제에서 headShape 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
 *
 * 기억할 점
 ...
 * - 노드가 Diagram에 등록되기 전에 연결선을 만들면 출발점이나 도착점이 올바르게 계산되지 않습니다.
 */
...
new Diagram();
    const start = /* [일반 노드 생성] ShapeNode 객체만 만든 상태이며, bounds·text·스타일을 설정한 뒤 Diagram에 등록해야 화면에 나타납니다. */
new ShapeNode(model); start.bounds = /* [위치와 크기] Rect(x, y, width, height)로 다이어그램 좌표상의 위치와 노드 크기를 함께 지정합니다. */
new Rect(18, 38, 52, 30); start.text = "출발"; model.addItem(start);
    const end = new ShapeNode(model); end.bounds = new Rect(120, 38, 52, 30); end.text = "도착"; model.addItem(end);
    const link = model.factory./* [연결선 생성] 출발 노드와 도착 노드가 Diagram에 준비된 뒤 두 노드를 연결합니다. */
createDiagramLink(start, end); link.headShape = ArrowHeads.Triangle(); link.headShapeSize = 5; link.stroke = "#d04f4f";
    return model;
  });
