/**
 * ================================================================
 * [초보자용 상세 주석] 11-04 컴포넌트 제거 시 이벤트 해제
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - cleanup에서 직접 등록한 모든 DOM 이벤트 리스너를 해제합니다.
 * - 예상 결과: 노드 1개, 연결선 0개
 ...
 * - useEffect cleanup: 이번 예제에서 useEffect cleanup 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
...
new Diagram()); const viewRef = useRef(null); const initialized = useRef(false); const listeners = useRef(new Map()); const [clicks, setClicks] = useState(0);
...
const domCreated = (_sender, args) => { const button = args.node.getContent().querySelector('[data-testid="cleanup-button"]'); if (!button || listeners.current.has(button)) return; const handler = () => setClicks((value) => { const next = value + 1; report(next); return next; }); button./* [브라우저 이벤트 등록] DOM 요소와 handler 함수 쌍을 기억해야 cleanup에서 정확히 같은 함수로 제거할 수 있습니다. */
addEventListener("click", handler); listeners.current.set(button, handler); report(0); };
  ...
  return <div data-testid="diagram-demo"><p data-testid="cleanup-status">등록 리스너 {listeners.current.size}개 · 클릭 {clicks}회 · 메뉴 이동 시 해제</p><div style={{ height: 460 }}>/* [화면 렌더링] 준비한 Diagram 모델과 ref, 이벤트 함수를 DiagramView prop으로 전달합니다. 부모 요소에는 반드시 높이가 있어야 합니다. */
