/**
 * ================================================================
 * [초보자용 상세 주석] 06-01 부모 1개와 자식 1개
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 부모 1개와 자식 1개를 연결해 가장 작은 트리를 만듭니다.
 * - 예상 결과: 노드 2개, 연결선 1개
 ...
 * - 화면상의 위치만 보고 부모·자식을 판단하지 말고 실제 연결선의 origin과 destination을 사용해야 합니다.
 */
...
new Diagram();
    const node = (text, x, y, expandable = false) => { const item = /* [일반 노드 생성] ShapeNode 객체만 만든 상태이며, bounds·text·스타일을 설정한 뒤 Diagram에 등록해야 화면에 나타납니다. */
new ShapeNode(model); item.bounds = /* [위치와 크기] Rect(x, y, width, height)로 다이어그램 좌표상의 위치와 노드 크기를 함께 지정합니다. */
new Rect(x, y, 52, 28); item.text = text; item.expandable = expandable; item.expanded = expandable; model.addItem(item); return item; };
    const parent = node("부모", 82, 20, true); const child = node("자식 1", 82, 92);
    model.factory./* [연결선 생성] 출발 노드와 도착 노드가 Diagram에 준비된 뒤 두 노드를 연결합니다. */
