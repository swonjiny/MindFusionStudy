/**
 * ================================================================
 * [초보자용 상세 주석] 08-06 특정 깊이까지만 펼치기
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 재귀 순회로 2단계까지만 노드와 연결선을 표시합니다.
 * - 예상 결과: 노드 7개, 연결선 6개
...
new Rect(x, y, 44, 23); n.text = text; n.expandable = expandable; n.expanded = false; model.addItem(n); return n; }; const root = make("1단계", 100, 8, true), a = make("2단계 A", 45, 75, true), b = make("2단계 B", 155, 75, true); root.tag = "root"; model.factory./* [연결선 생성] 출발 노드와 도착 노드가 Diagram에 준비된 뒤 두 노드를 연결합니다. */
createDiagramLink(root, a); model.factory.createDiagramLink(root, b); [make("3단계 A1", 15, 145), make("3단계 A2", 67, 145)].forEach((n) => model.factory.createDiagramLink(a, n)); [make("3단계 B1", 137, 145), make("3단계 B2", 189, 145)].forEach((n) => model.factory.createDiagramLink(b, n)); model.nodes.forEach((n) => { n.visible = n === root; }); model.links.forEach((l) => { l.visible = false; }); return model; });
  ...
  return <div data-testid="diagram-demo"><button onClick={() => expandToDepth(2)}>2단계까지만 펼치기</button><span data-testid="tree-state"> {done ? "깊이 2까지 표시" : "루트만 표시"}</span><div style={{ height: 460 }}>/* [화면 렌더링] 준비한 Diagram 모델과 ref, 이벤트 함수를 DiagramView prop으로 전달합니다. 부모 요소에는 반드시 높이가 있어야 합니다. */
