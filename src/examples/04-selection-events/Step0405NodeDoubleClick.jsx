/**
 * ================================================================
 * [초보자용 상세 주석] 04-05 노드 더블 클릭 이벤트
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - onNodeDoubleClicked에서 더블 클릭 이벤트를 확인합니다.
 * - 예상 결과: 노드 2개, 연결선 0개
 ...
 *
 * 이번 예제의 핵심 용어
 * - onNodeDoubleClicked: 이번 예제에서 onNodeDoubleClicked 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
...
new Diagram();
    ["노드 A", "노드 B"].forEach((text, index) => { const node = /* [일반 노드 생성] ShapeNode 객체만 만든 상태이며, bounds·text·스타일을 설정한 뒤 Diagram에 등록해야 화면에 나타납니다. */
new ShapeNode(model); node.bounds = /* [위치와 크기] Rect(x, y, width, height)로 다이어그램 좌표상의 위치와 노드 크기를 함께 지정합니다. */
new Rect(28 + index * 76, 36, 56, 34); node.text = text; model.addItem(node); });
    return model;
  });
...
<DiagramView diagram={diagram} behavior={Behavior.Modify} onNodeDoubleClicked={doubled} style={{ width: "100%", height: "100%" }} /></div></div>;
}
