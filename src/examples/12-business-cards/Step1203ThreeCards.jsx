/**
 * ================================================================
 * [초보자용 상세 주석] 12-03 명함형 카드 3개
 * ================================================================
 *
 * 이 파일에서 만드는 것
 * - 카드 세 개와 계산된 노드 크기를 확인합니다.
 * - 예상 결과: 노드 1개, 연결선 0개
 ...
 * - Array.length: 이번 예제에서 Array.length 기능을 설정하거나 실행하기 위해 사용하는 API·속성입니다.
...
const cards = [{ id: 1, nickname: "민트", intro: "UI 흐름을 설계합니다.", color: "#2b8a78" }, { id: 2, nickname: "라온", intro: "데이터 시각화를 만듭니다.", color: "#6f5bd3" }, { id: 3, nickname: "하루", intro: "사용자 경험을 연구합니다.", color: "#d06b45" }];
...
const cardHtml = (card) => `<article data-card-id="${card.id}" style="display:flex;align-items:center;gap:12px;padding:8px;border:1px solid #d7e3f4;border-radius:11px;background:#fff"><img src="${avatar(card)}" alt="${card.nickname} 프로필" style="width:40px;height:40px;border-radius:50%"/><div><strong style="display:block;color:#17345f;font:700 14px sans-serif">${card.nickname}</strong><span style="color:#61708a;font:12px sans-serif">${card.intro}</span></div></article>`;

/* [컴포넌트 시작] 이 함수가 외부에서 import해 렌더링하는 예제 컴포넌트입니다. props의 onStatus는 선택 사항입니다. */
export default function Step1203ThreeCards({ onStatus } = {}) {
...
new ControlNode(coreView); node.template = `<section data-mf-card-node="12-03" style="box-sizing:border-box;width:100%;height:100%;display:grid;gap:7px;padding:10px;border:2px solid #2877de;border-radius:14px;background:#f7fbff">${cards.map(cardHtml).join("")}</section>`; node.bounds = /* [위치와 크기] Rect(x, y, width, height)로 다이어그램 좌표상의 위치와 노드 크기를 함께 지정합니다. */
new Rect(52, 10, width, height); /* [모델에 등록] 직접 new로 만든 노드는 addItem을 호출해야 Diagram이 관리하고 DiagramView가 그릴 수 있습니다. */
...
const domCreated = (_sender, args) => report(args.node.getContent().querySelectorAll("[data-card-id]").length);
  ...
  return <div data-testid="diagram-demo"><p data-testid="card-summary">배열 데이터 {cards.length}개 · 카드 3개</p><div style={{ height: 460 }}>/* [화면 렌더링] 준비한 Diagram 모델과 ref, 이벤트 함수를 DiagramView prop으로 전달합니다. 부모 요소에는 반드시 높이가 있어야 합니다. */
